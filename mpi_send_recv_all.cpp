...
{    
    ...
    /*
        ...
        ******************************************************************
        ******************************************************************
        code to send/receive native particles between processors
        ******************************************************************
        ******************************************************************
        ******************************************************************
        ******************************************************************
    */
    ...
    // first make local_particles_native0 from local_particles, local_size_native0 from local_size
    int *local_size_native0 = (int *)malloc(sizeof(int)); 
    *local_size_native0 = 0;
    ...
    for (int i = 0; i < *local_size; i++){
            ...
            if ( proc_y == proc_y_current &&  proc_x == proc_x_current ){
                 ...
                 *local_size_native0 += 1;
            }
    }
    particle_t *local_particles_native0 = (particle_t*) malloc (*local_size_native0 * sizeof(particle_t));
    std::set<int>::iterator it1;
    int index_temp = 0;
    for (it1 = index_native.begin(); it1 != index_native.end(); ++it1){
        local_particles_native0[index_temp++] = local_particles[*it1];
    }
    std::cout<<"rank"<<rank<<"local_size_native0 "<<*local_size_native0 <<std::endl;


    // manually change the location of some particles to other processors for experiment
    if (rank == 0){
        for (int i = 0; i < 10; i++){
            local_particles_native0[i].y = get_size() - 0.000001 * (i+1);
            local_particles_native0[i].x = get_size() - 0.000001 * (i+1);
    ...
    if (rank == num_proc_x*num_proc_y - 1){
        for (int i = 0; i < 10; i++){
            local_particles_native0[i].x = 0 + 0.000001 * (i+1);
            local_particles_native0[i].y = 0 + 0.000001 * (i+1);
    ...
    // convert local_particles_native to a map, dummy
    std::map<double, particle_t> local_particles_native_map;
    for (int i = 0; i < *local_size_native0; i++){
        //std::cout <<"local_particles_native0[i].id "<< local_particles_native0[i].id <<std::endl;
        local_particles_native_map.insert(std::pair <double, particle_t> (local_particles_native0[i].id, local_particles_native0[i]));
    }
    //std::cout<<"rank"<<rank<<"local_particles_native_map.size() "<<local_particles_native_map.size() <<std::endl;
   ...
   if(proc_y_current - 1 >= 0 && proc_x_current - 1 >=0){
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    double tttt = *receive_size_up + *receive_size_upperleft + *receive_size_left + *receive_size_lowerleft;
    tttt += *receive_size_down + *receive_size_lowerright + *receive_size_right +*receive_size_upperright;
    std::cout<<"rank "<<rank<<" receive_size: " <<tttt<<std::endl;
    ...
    */
    ...
    /*
        ...
        ******************************************************************
        ******************************************************************
        code to send/receive neighboring particles between processors
        ******************************************************************
        ******************************************************************
        ******************************************************************
        ******************************************************************
    */
    ...
    /*
     convert the local_particles_native_map map to an array local_particles_native_new
     */
    ...
    */
    ...
    for (int i = 0; i < *local_size_native_new; i++){
         // may <= num_proc_x or < 0
            ...
            // process_ids_new = get_procs(local_particles_native_new[i].x, local_particles_native_new[i].y, num_proc_x, num_proc_y);
            // // the 0-th term is the current processor number
            // for (int j = 1; j < 9; j++){
            //     if(process_ids_new[j] != -1){ // this particle needs to be sent
            //         index_send_new.insert(i);
            //         break;
            //     }
            // }
            // // up
            // if (process_ids_new[1] != -1)
            //     *send_size_up_new += 1;
            // // down
            // if (process_ids_new[2] != -1)
            //     *send_size_down_new += 1;
            // // left
            // if (process_ids_new[3] != -1)
            //     *send_size_left_new += 1;
            // // right
            // if (process_ids_new[4] != -1)
            //     *send_size_right_new += 1;
            // // upperright
            // if (process_ids_new[5] != -1)
            //     *send_size_upperright_new += 1;
            // // upperleft
            // if (process_ids_new[6] != -1)
            //     *send_size_upperleft_new += 1;
            // // lowerleft
            // if (process_ids_new[7] != -1)
            //     *send_size_lowerleft_new += 1;
            // // lowerright
            // if (process_ids_new[8] != -1)
            //     *send_size_lowerright_new += 1;
    }
    //
    // std::cout<<"rank "<<rank<<" #neighboring particles to be sent up "<<*send_size_up_new<<std::endl;
    // std::cout<<"rank "<<rank<<" #neighboring particles to be sent down "<<*send_size_down_new<<std::endl;
    // std::cout<<"rank "<<rank<<" #neighboring particles to be sent left "<<*send_size_left_new<<std::endl;
    // std::cout<<"rank "<<rank<<" #neighboring particles to be sent right "<<*send_size_right_new<<std::endl;
    // std::cout<<"rank "<<rank<<" #neighboring particles to be sent upperright "<<*send_size_upperright_new<<std::endl;
    // std::cout<<"rank "<<rank<<" #neighboring particles to be sent upperleft "<<*send_size_upperleft_new<<std::endl;
    // std::cout<<"rank "<<rank<<" #neighboring particles to be sent lowerleft "<<*send_size_lowerleft_new<<std::endl;
    // std::cout<<"rank "<<rank<<" #neighboring particles to be sent upperright "<<*send_size_upperright_new<<std::endl;
    // // index_send_size_new should be the sum of the above 8 numbers
    ...
    */
    ...
    // if(rank == 0){
    //     std::cout<<"rank 0**************************************************"<<std::endl;
    //     std::cout<<"rank 0 number of nb particles received from up"<<*receive_size_up_new<<std::endl;
    //     std::cout<<"rank 0 number of nb particles received from upperleft"<<*receive_size_upperleft_new<<std::endl;
    //     std::cout<<"rank 0 number of nb particles received from left"<<*receive_size_left_new<<std::endl;
    //     std::cout<<"rank 0 number of nb particles received from lowerleft"<<*receive_size_lowerleft_new<<std::endl;
    //     std::cout<<"rank 0 number of nb particles received from down"<<*receive_size_down_new<<std::endl;
    //     std::cout<<"rank 0 number of nb particles received from lowerright"<<*receive_size_lowerright_new<<std::endl;
    //     std::cout<<"rank 0 number of nb particles received from right"<<*receive_size_right_new<<std::endl;
    //     std::cout<<"rank 0 number of nb particles received from upperright"<<*receive_size_upperright_new<<std::endl;
    // }
    // // should receive 10 points from left
    // if(rank == 1){
    //     std::cout<<"rank 1**************************************************"<<std::endl;
    //     std::cout<<"rank 1 number of nb particles received from up"<<*receive_size_up_new<<std::endl;
    //     std::cout<<"rank 1 number of nb particles received from upperleft"<<*receive_size_upperleft_new<<std::endl;
    //     std::cout<<"rank 1 number of nb particles received from left"<<*receive_size_left_new<<std::endl;
    //     std::cout<<"rank 1 number of nb particles received from lowerleft"<<*receive_size_lowerleft_new<<std::endl;
    //     std::cout<<"rank 1 number of nb particles received from down"<<*receive_size_down_new<<std::endl;
    //     std::cout<<"rank 1 number of nb particles received from lowerright"<<*receive_size_lowerright_new<<std::endl;
    //     std::cout<<"rank 1 number of nb particles received from right"<<*receive_size_right_new<<std::endl;
    //     std::cout<<"rank 1 number of nb particles received from upperright"<<*receive_size_upperright_new<<std::endl;
    // }
    // // should receive 10 points from right
    // if(rank == 2){
    //     std::cout<<"rank 2**************************************************"<<std::endl;
    //     std::cout<<"rank 2 number of nb particles received from up"<<*receive_size_up_new<<std::endl;
    //     std::cout<<"rank 2 number of nb particles received from upperleft"<<*receive_size_upperleft_new<<std::endl;
    //     std::cout<<"rank 2 number of nb particles received from left"<<*receive_size_left_new<<std::endl;
    //     std::cout<<"rank 2 number of nb particles received from lowerleft"<<*receive_size_lowerleft_new<<std::endl;
    //     std::cout<<"rank 2 number of nb particles received from down"<<*receive_size_down_new<<std::endl;
    //     std::cout<<"rank 2 number of nb particles received from lowerright"<<*receive_size_lowerright_new<<std::endl;
    //     std::cout<<"rank 2 number of nb particles received from right"<<*receive_size_right_new<<std::endl;
    //     std::cout<<"rank 2 number of nb particles received from upperright"<<*receive_size_upperright_new<<std::endl;
    // }
    // // should receive 10 points from lowerleft
    // if(rank == 3){
    //     std::cout<<"rank 3**************************************************"<<std::endl;
    //     std::cout<<"rank 3 number of nb particles received from up"<<*receive_size_up_new<<std::endl;
    //     std::cout<<"rank 3 number of nb particles received from upperleft"<<*receive_size_upperleft_new<<std::endl;
    //     std::cout<<"rank 3 number of nb particles received from left"<<*receive_size_left_new<<std::endl;
    //     std::cout<<"rank 3 number of nb particles received from lowerleft"<<*receive_size_lowerleft_new<<std::endl;
    //     std::cout<<"rank 3 number of nb particles received from down"<<*receive_size_down_new<<std::endl;
    //     std::cout<<"rank 3 number of nb particles received from lowerright"<<*receive_size_lowerright_new<<std::endl;
    //     std::cout<<"rank 3 number of nb particles received from right"<<*receive_size_right_new<<std::endl;
    //     std::cout<<"rank 3 number of nb particles received from upperright"<<*receive_size_upperright_new<<std::endl;
    // }
